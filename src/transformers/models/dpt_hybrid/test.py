...
"""Convert DptHybrid checkpoints from the original repository. URL: https://github.com/isl-org/DptHybrid"""
...
def get_dpt_hybrid_config(checkpoint_url):
...
def rename_key(name):
    if (
        ...
        name = name.replace("pretrained.model", "dpt_hybrid.encoder")
    if "pretrained.model" in name:
        name = name.replace("pretrained.model", "dpt_hybrid.embeddings")
    ...
    if "pretrained" in name:
        name = name.replace("pretrained", "dpt_hybrid")
    if "bn" in name:
...
def read_in_q_k_v(state_dict, config):
    for i in range(config.num_hidden_layers):
        ...
        state_dict[f"dpt_hybrid.encoder.layer.{i}.attention.attention.key.bias"] = in_proj_bias[
            config.hidden_size : config.hidden_size * 2
        ]
...
@torch.no_grad()
def convert_dpt_hybrid_checkpoint(checkpoint_url, pytorch_dump_folder_path, push_to_hub, model_name):
    """
    Copy/paste/tweak model's weights to our DptHybrid structure.
    """

    # define DptHybrid configuration based on URL
    config, expected_shape = get_dpt_hybrid_config(checkpoint_url)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--checkpoint_url",
        default="https://github.com/intel-isl/DptHybrid/releases/download/1_0/dpt_hybrid_large-midas-2f21e586.pt",
        type=str,
        help="URL of the original DptHybrid checkpoint you'd like to convert.",
    )
    parser.add_argument(
