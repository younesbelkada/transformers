...
class PLBartLearnedPositionalEmbedding(nn.Embedding):
    ...
    def __init__(self, num_embeddings: int, embedding_dim: int):
        ...
        self.position_ids = self.register_buffer("position_ids", torch.arange(0, 2048, dtype=torch.long))

    def forward(self, input_ids: torch.Tensor, past_key_values_length: int = 0):
        """`input_ids' shape is expected to be [bsz x seqlen]."""

        bsz, seq_len = input_ids.shape[:2]
        positions = self.position_ids[past_key_values_length : seq_len + past_key_values_length].expand(bsz, -1)
