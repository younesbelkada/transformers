...
from .configuration_pix2struct import Pix2StructConfig, Pix2StructDecoderConfig
...
class Pix2StructEncoderSelfOutput(nn.Module):
    """
    The residual connection is defined in Pix2StructEncoderLayer instead of here (as is the case with other models), due to the
...
class Pix2StructEncoderPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: Union[nn.Linear, nn.Conv2d, nn.LayerNorm]) -> None:
        ...
        elif isinstance(module, Pix2StructEncoderEmbeddings):
...
VIT_INPUTS_DOCSTRING = r"""
    Args:
        pixel_values (`torch.FloatTensor` of shape `(batch_size, num_channels, height, width)`):
            Pixel values. Pixel values can be obtained using [`AutoImageProcessor`]. See [`Pix2StructEncoderImageProcessor.__call__`]
            for details.
...
class Pix2StructDecoderDenseActDense(nn.Module):
    def __init__(self, config: Pix2StructDecoderConfig):
        super().__init__()
...
class Pix2StructDecoderAttention(nn.Module):
    def __init__(self, config: Pix2StructDecoderConfig, has_relative_attention_bias=False):
        super().__init__()
...
class Pix2StructDecoderLayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        super().__init__()
        self.SelfAttention = Pix2StructDecoderAttention(config, has_relative_attention_bias=has_relative_attention_bias)
        self.layer_norm = Pix2StructDecoderLayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class Pix2StructDecoderLayerCrossAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.EncDecAttention = Pix2StructDecoderAttention(config, has_relative_attention_bias=False)
        self.layer_norm = Pix2StructDecoderLayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class Pix2StructDecoderBlock(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        ...
        self.layer.append(Pix2StructDecoderLayerFF(config))
...
class Pix2StructDecoderPreTrainedModel(PreTrainedModel):
    ...
    config_class = Pix2StructDecoderConfig
...
Pix2StructDecoder_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`Pix2StructDecoderConfig`]): Model configuration class with all the parameters of the model.
            Initializing with a config file does not load the weights associated with the model, only the
            configuration. Check out the [`~PreTrainedModel.from_pretrained`] method to load the model weights.
"""
...
Pix2StructDecoder_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`):
            Indices of input sequence tokens in the vocabulary. Pix2StructDecoder is a model with relative position embeddings so you
            ...
            To know more on how to prepare `input_ids` for pretraining take a look a [Pix2StructDecoder Training](./t5#training).
        ...
        decoder_input_ids (`torch.LongTensor` of shape `(batch_size, target_sequence_length)`, *optional*):
            ...
            Pix2StructDecoder uses the `pad_token_id` as the starting token for `decoder_input_ids` generation. If `past_key_values`
...
class Pix2StructForConditionalGeneration(Pix2StructDecoderPreTrainedModel):
    ...
    def __init__(self, config: Pix2StructConfig):
        ...
        self.decoder = Pix2StructDecoderStack(decoder_config, self.shared)
